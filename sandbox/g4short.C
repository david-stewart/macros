...
int g4short(
    ...
    const string &outputFile = "g4short_out.root",
    ...
    const string &outdir = ".")
{
  ...
  if (Input::HEPMC)
  {
    ...
    {
      watch (Form(" YES Input::PILEUPRATE > 0 (is it %10.5f ",Input::PILEUPRATE));
  ...
  watch ( Form(" Input::READHITS (%i)",Input::READHITS) );
  if (! Input::READHITS)
  {
    watch("In Input::READHITS");
...
//  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar field strength of 1.4T
  ...
  if(Enable::TRACKING_TRACK)
    {
      watch("Before TrackingInit()");
      TrackingInit();
      watch("After TrackingInit()");
    }
  ...
  watch(Form("Enable::DSTOUT  %i",Enable::DSTOUT));
  if (Enable::DSTOUT)
  {
  ...
  watch(Form("se->skip(%i)",skip));
  se->skip(skip);
  watch(Form("se->run(%i)",nEvents));
  se->run(nEvents);
  watch("Done with se->run(nEvents)");
